/**
 * Usando una promesa comprobar si todos los numeros de una lista son positivos, en tal caso 
 * llamar a resolve, si el array contiene algun numero negativo debe llamar a la funcion reject.
 * 
 * La funcion resolve debe mostrar el array por consola.
 * La funcion reject debe mostrar el numero negativo encontrado.
 * 
 * Ejemplo 1: para el array [1, 2, 3] debe llamar a resolve y mostrar consola [1, 2, 3]
 * Ejemplo 2: para el array [1, 2, -1] debe llamar a reject y mostrar por consola -1
 */

const lista=[8,4,9,12,32,9,58,14];
const listaNegativo=[8,4,9,12,32,9,58,-14];

function comprobarNegativo(listado) {
    let p = new Promise(function(resolve, reject) {
        let encontrado=false;
        for(let i=0;i<listado.length;i++){
            if(listado[i]<0){
                encontrado=true;
                 reject(listado[i]);
            }
        } if(!encontrado){
            resolve(listado);
        }});
    return p;
}

let consola=(listado)=>{
    console.log(listado);
}
let negativo=(num)=>{
    console.log(num);
}

comprobarNegativo(lista).then(consola).catch(negativo);
comprobarNegativo(listaNegativo).then(consola).catch(negativo);
comprobarNegativo(lista).then((listado)=>console.log(listado)).catch((num)=>console.log(num));
comprobarNegativo(listaNegativo).then((listado)=>console.log(listado)).catch((num)=>console.log(num));